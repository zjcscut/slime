...
public class ZookeeperDistributedLockFactory implements DistributedLockFactory, DisposableBean {
    ...
    private ZookeeperClientProperties properties;
    ...
    private void lazyInitZookeeperClient() {
        try {
            ...
            if (null == properties) {
                ...
                properties = clientConfiguration.getZookeeperClientProperties();
                Assert.notNull(properties, "Zookeeper client properties must not be null!!!");
                this.baseLockPath = properties.getBaseLockPath();
                Assert.hasText(baseLockPath, "Distributed lock baseLockPath must not be blank!!!");
            }
            if (TEST_MODE_SIGN.equals(location)) {
                initTestClient();
            } else {
                initRealClient(properties);
            }
        } catch (Exception e) {
    ...
    private void initRealClient(ZookeeperClientProperties clientProperties) {
        if (null == client) {
